...
SampleOrder = [
    "Samp_A",
    "Samp_C",
]  # , 'Samp_B', 'Samp_C'] # Order that you want to run samples.
# Motor positions
...
ZFlipPosition = [
    ...
    0.15,
    0,
]  # 'Sample Z' motor position when 'Sample Theta' is flipped 180deg
ReverseHolder = [
    0,
    0,
    0,
]  # Set to True (1) for a sample that is on the reverse side of the holder
...
SampleThickness = [
    ...
    250,
    250,
]  # Approximate film thickness [A], used to determine dq
...
OverlapPoints = [
    ...
    3,
    3,
]  # Number of overlap points upon changing motor. Used for stitching
CheckUncertainty = [
    2,
    2,
    2,
]  # Number of points for assessing error at each change in motor conditions. Used for error reduction
HOSBuffer = [
    2,
    2,
    2,
]  # Number of points to add upon changing HOS to account for  motor movement.
I0Points = [
    10,
    10,
    10,
]  # Number of I0 measureents taken at the start of the scan. Used to calculate direct beam uncertainty
...
for deg, hos, hes, exp in motors:
    temp_list.append(
        pd.DataFrame({"Angle": deg, "HOS": hos, "HES": hes, "EXPOSURE": exp})
    )
