...
def postprocess_fit(fitter, burn_thresh=0.005, convergence_thresh=0.95, numpnts=512000):
    ...
    chainmin_lp = np.max(logpost, axis=0) #Its a negative number - individual chain minima
...
def plot_PSoXR(objective):
    ...
    for i, objective in enumerate(objective.objectives):
        ...
        if datapol[0] == 'fit':
            ...
            swap_loc = [np.argmax(np.abs(np.diff(data.data[0]))) + 1, -1] #Run up until the last value in the polarization
            datapol = ['s','p']
        else:
            init_loc = [0, 0]
            swap_loc = [-1, -1] #If its a single polarization we will only have a single polarization
        ...
        for i, pol in enumerate(pol_list):
            if pol in datapol:
                polname = str(pol + 'pol')
                model = ani_ReflectModel(structure, scale=scale, bkg=bkg, dq=dq, energy=energy, pol=pol, name=polname)
                ...
                save_model_columns = ['qvals', str('refl_'+pol+'pol'), str('reflu_'+pol+'pol'), str('model_'+pol+'pol'), str('res_'+pol+'pol')]
                save_model = pd.DataFrame(np.array([qvals, refl, reflu, model_refl, model_res]).T, columns=save_model_columns)
                save_model_columns_interp = ['model_qvals', str('model_'+pol+'pol')]
                save_model_interp = pd.DataFrame(np.array([prettymodel_qvals, prettymodel_refl]).T, columns=save_model_columns_interp)
                save_model.to_csv((savename))
                save_model_interp.to_csv((savename_interp))
                
                #Plot results:
                res_ax.scatter(qvals, model_res, color='w', marker='o', s=3, edgecolors=pol_color[i], linewidth=0.5, label=str(pol+'-pol'))
                refl_ax.errorbar(qvals, refl, reflu, color=pol_color[i], marker='o', ms=3, ls="", markerfacecolor='w', markeredgewidth=0.5, zorder=10, label=str(pol+'-pol'))
...
def plot_depthprofile(objective):
    ...
    for i, objective in enumerate(objective.objectives):
        ...
        df.plot(ax=ax1, y='dxx', x='zed',style=['-.'], color=['#FF118C'], lw=1); df.plot(ax=ax1, y='dzz', x='zed', style=['--'],color=['#D55E00'], lw=1)
        df.plot(ax=ax1, x='zed', y='delta_trace', color=['#0072B2'], lw=1.5, legend=False)
        df.plot(ax=ax2, y='birefringence', x='zed', color=['#009E73'], legend=False)

        df.plot(ax=ax3, x='zed', y='bxx', style=['-.'], color=['#FF118C'], lw=1); df.plot(ax=ax3, x='zed', y='bzz', style=['--'], color=['#D55E00'], lw=1)
        df.plot(ax=ax3, x='zed', y='beta_trace', color=['#0072B2'], lw=1.5, legend=False)
...
def save_fitresults(objective, name='FitResults'):
    ...
    for objective in objective.objectives:
        ...
        save_columns = ['value', 'stderr', 'vary', 'prior_lb', 'prior_up', 'logprior', 'constraint', 'norm_mu', 'norm_std']
        ...
        for param in params_with_chain:
            ...
            save_output = save_output.append(pd.DataFrame([[value, stderr, vary, plb, pub, logp, con, mu, std]], columns=save_columns))
...
def MCMC_summary(fitter, nburn=0, nthin=1):
    ...
    az_summary = az.summary(az_sampler.posterior.sel(draw=slice(nburn,None,nthin)),var_names=variable_names, round_to=None)
