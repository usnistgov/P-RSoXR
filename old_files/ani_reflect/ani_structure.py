...
from ani_reflect.ani_reflect_model import PXR_reflectivity
...
The prefix 'PXR' (Resonant X-ray Reflectivity) will designate the objects required for working with polarized resonant soft X-ray reflectivity data. See class PXR_SLD for information on open tensor parameters.
...
class PXR_Structure(UserList):
    ...
    >>> from PRSoXR import PXR_SLD, PXR_MaterialSLD
    >>> # make the material with tensor index of refraction
    >>> vac = PXR_MaterialSLD('', density=1, name='vac')
    >>> polymer = PXR_SLD(np.array([complex(0.001, 0.002), complex(0.001, 0.002), complex(0.002, 0.001)]), name='Polymer')
    ...
    def slabs(self, **kwds):
        ...
        is done. I.e. if `PXR_Structure.solvent == 'backing'` and
        `PXR_Structure.reversed is True` then the material that solvates the system
        is the component in `Structure[0]`, which corresponds to
        `PXR_Structure.slab[-1]`.
...
class PXR_SLD(PXR_Scatterer):
    ...
    def __init__(self, value, name=''):
        ...
        self._parameters.extend([self.delta,self.beta,self.xx,self.ixx,self.yy,self.iyy,self.zz,self.izz,self.birefringence,self.dichroism])
...
class PXR_Slab(PXR_Component):
    ...
    PXR_sld : :class:`refnx.reflect.PXR_Scatterer`, complex, or float
