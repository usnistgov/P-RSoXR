...
def save_fitresults(path, objective, save_name):
    ...
    filename = save_name + '_ModelResults.xlsx';
    ...
    with pd.ExcelWriter(os.path.join(path + filename)) as writer:
        for objective in objective.objectives:
            ...
            for param in params_with_chain:
                ...
                prior_pdf = 1/(param.bounds.ub - param.bounds.lb)

                out = out.append(pd.DataFrame([[value, std, vary, logp, mu, std, prior_pdf]], columns=cols))
            ...
            out.to_excel(writer, sheet_name=(name+'_ModelValues'))
            ...
            hist_output.to_excel(writer, sheet_name=(name+'_Histogram'))
...
def save_PSoXR(path, objective, save_name):
    ...
    filename = save_name + '_ProfileFits.xlsx'
    ...
    with pd.ExcelWriter(os.path.join(path + filename)) as writer:
        for i, objective in enumerate(objective.objectives):
            ...
            datapol = list(objective.model.pol) # Check polarization associated with objective
            ...
            for i, pol in enumerate(pol_list):        # Possible polarizaitons
                if pol in datapol:
                    polname = str(pol + 'pol')
                    model = PXR_ReflectModel(structure, scale=scale, bkg=bkg, dq=dq, energy=energy, pol=pol, name=(name + '_' +polname))
                    ...
                    model_refl = model.model(data_qvals)
                    ...
                    fig.suptitle(name + '_' + energy_name + '_' + pol + 'pol', fontsize=8)
                    grid = fig.add_gridspec(5, 5)
                    refl_ax = fig.add_subplot(grid[1:,:])
                    res_ax = fig.add_subplot(grid[0,:],sharex=refl_ax)

                    # Append Data
                    res_ax.scatter(data_qvals, model_res, color='w', marker='o', s=3, edgecolors=pol_color[i], linewidth=0.5, label=str(pol+'pol'))
                    refl_ax.errorbar(data_qvals, data_refl, data_err, color=pol_color[i], marker='o', ms=3, ls="", markerfacecolor='w', markeredgewidth=0.5, zorder=10, label=str(pol+'pol'))
                    refl_ax.plot(data_qvals, model_refl, color='0', lw=1, zorder=30, label='fit')
...
def save_depthprofile(path, objective, save_name):
    ...
    with pd.ExcelWriter(os.path.join(path + filename)) as writer:
        ...
        for i, objective in enumerate(objective.objectives):
            ...
            zed, prof = objective.model.structure.sld_profile(align=-1) #calculate SLD profile
            iso_prof = prof.sum(axis=1)/3
            diff_prof = prof[:,0] - prof[:,2]

            #Compile components for dataframe
            depthprofile = {
                'zed' : -1*zed, 'delta_trace' : np.real(iso_prof), 'beta_trace' : np.imag(iso_prof), 'birefringence': np.real(diff_prof), 'dichroism' : np.imag(diff_prof),
